...
public class OCRImage {
    ...
    public OCRImage(BufferedImage image) {
       ...
       try{
           tempFile = Files.createTempFile("tempFile", ".png");
           ImageIO.write(grayscaleImage, "png", tempFile.toFile());
           ImageIO.write(grayscaleImage, "png", new File("tempImage.png"));
...
//    }
    private void grayscaleImage(BufferedImage image) {
        ...
        GrayF32 denoise = gray.createSameShape();
        GBlurImageOps.gaussian(gray, denoise, -1, 2, null);

        BufferedImage tempBuf = new BufferedImage(
                denoise.width, denoise.height, BufferedImage.TYPE_BYTE_GRAY
        );
        ConvertBufferedImage.convertTo(denoise, tempBuf, true);
        GrayU8 gray8 = new GrayU8(denoise.width, denoise.height);
        ConvertBufferedImage.convertFrom(tempBuf, gray8, true);
        ...
        for (int y = 0; y < gray8.height; y++) {
            for (int x = 0; x < gray8.width; x++) {
                float normalized = (gray8.get(x, y) & 0xFF) / 255.0f;
                float curved = (float) Math.pow(normalized, 1.50);
                gamma.set(x, y, (int)(curved * 255));
            }
        }
        ...
        grayscaleImage = ConvertBufferedImage.convertTo(sharpened, null, true);
    }
}
