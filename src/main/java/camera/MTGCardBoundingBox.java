...
public class MTGCardBoundingBox extends JPanel {
    ...
    public BufferedImage detectAndDraw(BufferedImage input){
        ...
        if(smoother.isSmoothed()){
            if(ocrBox == null){
                double[] boxValues = smoother.getExactBox();
                var ocrX = (int)boxValues[0];
                var ocrY = (int)boxValues[1];
                var ocrWidth = (int)boxValues[2];
                var ocrHeight = (int)boxValues[3];
//                int ocrX = displayBox.x;
//                int ocrY = (int)(0.9 * displayBox.height);
//                int ocrWidth = (int)(0.5 * displayBox.width);
//                int ocrHeight = (int)(0.1 * displayBox.height);

                ocrBox = new Rectangle(ocrX, ocrY + (int)(0.9 * ocrHeight), (int)(0.50 * ocrWidth), (int)(0.1 * ocrHeight));
                ocrImage = input.getSubimage(ocrBox.x, ocrBox.y, ocrBox.width, ocrBox.height);
...
//    public void detectAndDraw(BufferedImage input){
//        int width = input.getWidth();
//        int height = input.getHeight();
//        double frameArea = width * height;
//
...
//        }
        ...
        // Compute scale to fit image in panel while preserving aspect ratio
        double scale = Math.min((double) panelW / imgW, (double) panelH / imgH);

        int scaledW = (int)(imgW * scale);
        int scaledH = (int)(imgH * scale);

        // Center the image in the panel — same as WebcamPanel behavior
        int offsetX = (panelW - scaledW) / 2;
        int offsetY = (panelH - scaledH) / 2;

        g.drawImage(displayImage, offsetX, offsetY, scaledW, scaledH, null);
    }
}
